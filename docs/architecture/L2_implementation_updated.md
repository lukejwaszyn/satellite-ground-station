...
│
├── hmi/                           # Mission Operations Interface
│   ├── generate_orbital_data.py   # Skyfield SGP4 backend (multi-constellation)
│   └── satellite-viz.html         # 3D mission operations frontend (Three.js)
│
├── cpp/
│   ├── src/
...
```
Celestrak (HTTPS, multiple groups) → TLE cache files
                 ↓
...
### 6. Mission Operations HMI Flow
```
Celestrak TLE API (weather, stations, resource groups)
        ↓
    generate_orbital_data.py (Skyfield SGP4 backend)
        ├→ Fetch TLEs from multiple groups with 24h caching
        ├→ Individual NORAD ID fallback for missing satellites
        ├→ Match by NORAD catalog number (primary) or normalized name
        ├→ Propagate 12 satellites at 30s intervals for 24h window
        ├→ Detect passes, compute Doppler profiles
        └→ Export orbital_data.json
        ↓
    python -m http.server 8080 (local web server)
        ↓
    satellite-viz.html (Three.js 3D frontend)
        ├→ Load orbital_data.json via HTTP fetch
        ├→ Render 3D globe with satellite positions
        ├→ Interpolate positions for smooth animation
        ├→ Display pass timeline, elevation/Doppler charts
        ├→ Sort and display satellites by role (primary → weather → display)
        └→ Fallback to demo mode if JSON unavailable
```
...
### HMI Stack
- **Three.js r128:** 3D globe rendering, satellite meshes, orbital trails (WebGL)
- **HTML5 Canvas:** Elevation profile and Doppler shift chart rendering
- **Vanilla JavaScript:** UI state management, satellite interpolation, time simulation
- **Skyfield (Python):** SGP4 orbital propagation backend for generate_orbital_data.py
- **NumPy:** ECEF coordinate computation, Doppler velocity calculations
...
```json
{
  ...
  "satellites": {
    "NOAA-20": {"frequency_hz": 137100000, "active": true, "role": "primary"},
    "NOAA-21": {"frequency_hz": 137100000, "active": true, "role": "weather"},
    "NOAA-15": {"frequency_hz": 137620000, "active": true, "role": "weather"},
    "NOAA-18": {"frequency_hz": 137912500, "active": true, "role": "weather"},
    "NOAA-19": {"frequency_hz": 137100000, "active": true, "role": "weather"}
  },
  "capture": {
  ...
  "paths": {
    ...
    "orbital_data": "orbital_data.json"
  }
}
```
